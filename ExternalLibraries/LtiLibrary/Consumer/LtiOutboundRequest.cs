...
namespace LtiLibrary.Consumer
{
    public class LtiOutboundRequest : LtiRequest
    {
        ...
        private void AddOptionalParametersToRequest(OAuthParameters parameters)
        {
            ...
            parameters.AddParameter(LtiConstants.LisPersonContactEmailPrimaryParameter, LisPersonContactEmailPrimary);
        ...
        private string SubstituteCustomValue(string value)
        {
            ...
            value = Regex.Replace(value, "\\$User.id",
                UserId ?? string.Empty, RegexOptions.IgnoreCase);

            // LIS Person variables
            value = Regex.Replace(value, "\\$Person.sourcedId",
                UserId ?? string.Empty, RegexOptions.IgnoreCase);
            value = Regex.Replace(value, "\\$Person.name.full",
                LisPersonNameFull ?? string.Empty, RegexOptions.IgnoreCase);
            value = Regex.Replace(value, "\\$Person.name.family",
                LisPersonNameFamily ?? string.Empty, RegexOptions.IgnoreCase);
            value = Regex.Replace(value, "\\$Person.name.given",
                LisPersonNameGiven ?? string.Empty, RegexOptions.IgnoreCase);
            value = Regex.Replace(value, "\\$Person.email.primary",
                LisPersonContactEmailPrimary ?? string.Empty, RegexOptions.IgnoreCase);
