...
PRINT 'Creating PROCEDURE uspGetResultsFromInstitutionForChart'
GO
...
/*============================================================================================================          
 ...
 -- Purpose: To add the newly added category      
 -- Modified: 04/9/2013 ,10/28/2013     
 -- Author: Liju ,Liju       
 ******************************************************************************          
 ...
 *****************************************************************************/        
 ...
 BEGIN    
  ...
  WHERE (dbo.Norm.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))  ) and (ChartType= @ChartType)) B ON B.LevelOfDifficultyID = dbo.Questions.LevelOfDifficultyID    
  WHERE   ProductID IN ( select value from  dbo.funcListToTableIntForReport(@TestTypeIds,'|'))    
  AND TestStatus = 1    
  AND InsitutionID IN ( select value from  dbo.funcListToTableIntForReport(@InstitutionIds,'|'))    
 ...
 BEGIN    
  ...
  WHERE (dbo.Norm.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))  )  and (ChartType= @ChartType)) B ON B.NursingProcessID = dbo.Questions.NursingProcessID    
  WHERE   ProductID IN ( select value from  dbo.funcListToTableIntForReport(@TestTypeIds,'|'))    
  AND TestStatus = 1    
  AND InsitutionID IN ( select value from  dbo.funcListToTableIntForReport(@InstitutionIds,'|'))    
  AND dbo.UserTests.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))    
 ...
 BEGIN    
  ...
  WHERE (dbo.Norm.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))  ) and (ChartType= @ChartType)) B ON B.ClinicalConceptID = dbo.Questions.ClinicalConceptsID    
  WHERE   ProductID IN ( select value from  dbo.funcListToTableIntForReport(@TestTypeIds,'|'))    
  AND TestStatus = 1    
  AND InsitutionID IN ( select value from  dbo.funcListToTableIntForReport(@InstitutionIds,'|'))    
  AND dbo.UserTests.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))    
 ...
 BEGIN    
  ...
  WHERE (dbo.Norm.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))  )  and (ChartType= @ChartType) ) B ON B.ClientNeedsID = dbo.Questions.ClientNeedsID    
  WHERE   ProductID IN ( select value from  dbo.funcListToTableIntForReport(@TestTypeIds,'|'))    
  AND TestStatus = 1    
  AND InsitutionID IN ( select value from  dbo.funcListToTableIntForReport(@InstitutionIds,'|'))    
  AND dbo.UserTests.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))    
 ...
 BEGIN    
  ...
  WHERE (dbo.Norm.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))  )  and (ChartType= @ChartType)) B ON B.ClientNeedCategoryID = dbo.Questions.ClientNeedsCategoryID    
  WHERE   ProductID IN ( select value from  dbo.funcListToTableIntForReport(@TestTypeIds,'|'))    
  AND TestStatus = 1    
  AND InsitutionID IN ( select value from  dbo.funcListToTableIntForReport(@InstitutionIds,'|'))    
  AND dbo.UserTests.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))    
 ...
 BEGIN    
  ...
  WHERE (dbo.Norm.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))  )  and (ChartType= @ChartType)) B ON B.DemographicID = dbo.Questions.DemographicID    
  WHERE   ProductID IN ( select value from  dbo.funcListToTableIntForReport(@TestTypeIds,'|'))    
  AND TestStatus = 1    
  AND InsitutionID IN ( select value from  dbo.funcListToTableIntForReport(@InstitutionIds,'|'))    
  AND dbo.UserTests.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))    
 ...
 BEGIN    
  ...
  WHERE (dbo.Norm.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))  )  and (ChartType= @ChartType)) B ON B.CognitiveLevelID = dbo.Questions.CognitiveLevelID    
  WHERE   ProductID IN ( select value from  dbo.funcListToTableIntForReport(@TestTypeIds,'|'))    
  AND TestStatus = 1    
  AND InsitutionID IN ( select value from  dbo.funcListToTableIntForReport(@InstitutionIds,'|'))    
  AND dbo.UserTests.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))    
 ...
 BEGIN    
  ...
  WHERE (dbo.Norm.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))  )  and (ChartType= @ChartType)) B ON B.CriticalThinkingID = dbo.Questions.CriticalThinkingID    
  WHERE   ProductID IN ( select value from  dbo.funcListToTableIntForReport(@TestTypeIds,'|'))    
  AND TestStatus = 1    
  AND InsitutionID IN ( select value from  dbo.funcListToTableIntForReport(@InstitutionIds,'|'))    
  AND dbo.UserTests.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))    
 ...
 BEGIN    
  ...
  WHERE (dbo.Norm.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))  )  and (ChartType= @ChartType)) B ON B.SpecialtyAreaID = dbo.Questions.SpecialtyAreaID    
  WHERE   ProductID IN ( select value from  dbo.funcListToTableIntForReport(@TestTypeIds,'|'))    
  AND TestStatus = 1    
  AND InsitutionID IN ( select value from  dbo.funcListToTableIntForReport(@InstitutionIds,'|'))    
  AND dbo.UserTests.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))    
 ...
 BEGIN    
  ...
  WHERE (dbo.Norm.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))  )  and (ChartType= @ChartType)) B ON B.SystemID = dbo.Questions.SystemID    
  WHERE   ProductID IN ( select value from  dbo.funcListToTableIntForReport(@TestTypeIds,'|'))    
  AND TestStatus = 1    
  AND InsitutionID IN ( select value from  dbo.funcListToTableIntForReport(@InstitutionIds,'|'))    
  AND dbo.UserTests.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))    
 ...
 BEGIN    
  SELECT    SUM(CASE WHEN correct = 1 THEN 1 ELSE 0 END) AS Correct_N, SUM(1) AS Total    
  , dbo.AccreditationCategories.AccreditationCategories as ItemText,  dbo.CategoryPercentage.Percentage    
  , B.Norm,dbo.NurInstitution.InstitutionName    
  FROM  dbo.UserQuestions    
  INNER JOIN dbo.UserTests ON dbo.UserTests.UserTestID=dbo.UserQuestions.UserTestID    
  INNER JOIN dbo.NurInstitution ON dbo.UserTests.InsitutionID=dbo.NurInstitution.InstitutionID    
  ...
  LEFT OUTER JOIN  dbo.CategoryPercentage ON dbo.CategoryPercentage.TestID = dbo.UserTests.TestID AND dbo.CategoryPercentage.CategoryFields = dbo.AccreditationCategories.AccreditationCategoriesID AND dbo.CategoryPercentage.Category = 5    
  ...
  WHERE (dbo.Norm.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))  )  and (ChartType= @ChartType)) B ON B.AccreditationCategoriesID = dbo.Questions.AccreditationCategoriesID    
  WHERE   ProductID IN ( select value from  dbo.funcListToTableIntForReport(@TestTypeIds,'|'))    
  AND TestStatus = 1    
  AND InsitutionID IN ( select value from  dbo.funcListToTableIntForReport(@InstitutionIds,'|'))    
  AND dbo.UserTests.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))    
  ...
  AND (RTRIM(LTRIM(@ToDate)) = '' OR (ISDATE(@ToDate) > 0    
  AND (dbo.UserTests.TestStarted <= CONVERT(DATETIME,@ToDate))))    
  GROUP BY dbo.AccreditationCategories.AccreditationCategories, dbo.CategoryPercentage.Percentage    
  , B.Norm,dbo.NurInstitution.InstitutionName                      
  ORDER BY dbo.AccreditationCategories.AccreditationCategories,dbo.NurInstitution.InstitutionName    
 END     
 ELSE IF (@ChartType = 'QSENKSACompetencies')    
 BEGIN    
  SELECT    SUM(CASE WHEN correct = 1 THEN 1 ELSE 0 END) AS Correct_N, SUM(1) AS Total    
  , dbo.QSENKSACompetencies.QSENKSACompetencies as ItemText,  dbo.CategoryPercentage.Percentage    
  , B.Norm,dbo.NurInstitution.InstitutionName    
  FROM  dbo.UserQuestions    
  INNER JOIN dbo.UserTests ON dbo.UserTests.UserTestID=dbo.UserQuestions.UserTestID    
  INNER JOIN dbo.NurInstitution ON dbo.UserTests.InsitutionID=dbo.NurInstitution.InstitutionID    
  ...
  LEFT OUTER JOIN  dbo.CategoryPercentage ON dbo.CategoryPercentage.TestID = dbo.UserTests.TestID AND dbo.CategoryPercentage.CategoryFields = dbo.QSENKSACompetencies.QSENKSACompetenciesID AND dbo.CategoryPercentage.Category = 5    
  ...
  WHERE (dbo.Norm.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))  )  and (ChartType= @ChartType)) B ON B.QSENKSACompetenciesID = dbo.Questions.QSENKSACompetenciesID    
  WHERE   ProductID IN ( select value from  dbo.funcListToTableIntForReport(@TestTypeIds,'|'))    
  AND TestStatus = 1    
  AND InsitutionID IN ( select value from  dbo.funcListToTableIntForReport(@InstitutionIds,'|'))    
  AND dbo.UserTests.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))    
  ...
  AND (RTRIM(LTRIM(@ToDate)) = '' OR (ISDATE(@ToDate) > 0    
  AND (dbo.UserTests.TestStarted <= CONVERT(DATETIME,@ToDate))))    
  GROUP BY dbo.QSENKSACompetencies.QSENKSACompetencies, dbo.CategoryPercentage.Percentage    
  , B.Norm,dbo.NurInstitution.InstitutionName                      
  ORDER BY dbo.QSENKSACompetencies.QSENKSACompetencies,dbo.NurInstitution.InstitutionName    
 END     
 ELSE IF (@ChartType = 'Concepts')    
 BEGIN    
  SELECT    SUM(CASE WHEN correct = 1 THEN 1 ELSE 0 END) AS Correct_N, SUM(1) AS Total    
  , dbo.Concepts.Concepts as ItemText,  dbo.CategoryPercentage.Percentage    
  , B.Norm,dbo.NurInstitution.InstitutionName    
  FROM  dbo.UserQuestions    
  INNER JOIN dbo.UserTests ON dbo.UserTests.UserTestID=dbo.UserQuestions.UserTestID    
  INNER JOIN dbo.NurInstitution ON dbo.UserTests.InsitutionID=dbo.NurInstitution.InstitutionID    
  ...
  LEFT OUTER JOIN  dbo.CategoryPercentage ON dbo.CategoryPercentage.TestID = dbo.UserTests.TestID AND dbo.CategoryPercentage.CategoryFields = dbo.Concepts.ConceptsID AND dbo.CategoryPercentage.Category = 5    
  LEFT OUTER JOIN (SELECT dbo.Norm.Norm, dbo.Concepts.ConceptsID    
  FROM dbo.Norm INNER JOIN    
  dbo.Concepts ON    
  dbo.Norm.ChartID = dbo.Concepts.ConceptsID    
  WHERE (dbo.Norm.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))  )  and (ChartType= @ChartType)) B ON B.ConceptsID = dbo.Questions.ConceptsID    
  WHERE   ProductID IN ( select value from  dbo.funcListToTableIntForReport(@TestTypeIds,'|'))    
  AND TestStatus = 1    
  AND InsitutionID IN ( select value from  dbo.funcListToTableIntForReport(@InstitutionIds,'|'))    
  AND dbo.UserTests.TestID IN ( select value from  dbo.funcListToTableIntForReport(@TestIds,'|'))    
  ...
  AND (RTRIM(LTRIM(@ToDate)) = '' OR (ISDATE(@ToDate) > 0    
  AND (dbo.UserTests.TestStarted <= CONVERT(DATETIME,@ToDate))))    
  GROUP BY dbo.Concepts.Concepts, dbo.CategoryPercentage.Percentage    
  , B.Norm,dbo.NurInstitution.InstitutionName                      
  ORDER BY dbo.Concepts.Concepts,dbo.NurInstitution.InstitutionName    
 END     
SET NOCOUNT OFF   
END   
GO

PRINT 'Finished creating PROCEDURE uspGetResultsFromInstitutionForChart'
GO 
