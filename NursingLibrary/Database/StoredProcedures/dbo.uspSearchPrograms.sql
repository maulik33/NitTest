SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[uspSearchPrograms]') AND type in (N'P', N'PC'))
    DROP PROCEDURE [dbo].[uspSearchPrograms]
GO
CREATE PROC [dbo].[uspSearchPrograms]
(
	@ProgramofStudyId INT = 0,
	@SearchText varchar(200)
)
AS
BEGIN
SET NOCOUNT ON;
/*============================================================================================================                
 -- Purpose: Search Programs by specified search string
 -- Modified to return programs associated with ProgramOfStudy
 -- Modified: 10/10/2011,06/01/2013              
 -- Author:Glenn,Maulik       
 ******************************************************************************                
 * This software is the confidential and proprietary information of                
 * Kaplan,Inc. ("Confidential Information").  You shall not                
 * disclose such Confidential Information and shall use it only in                
 * accordance with the terms of the license agreement you entered into                
 * with Kaplan, Inc.                
 *                
 * KAPLAN, INC. MAKES NO REPRESENTATIONS OR WARRANTIES ABOUT THE                 
 * SUITABILITY OF THE SOFTWARE, EITHER EXPRESS OR IMPLIED, INCLUDING BUT                 
 * NOT LIMITED TO THE IMPLIED WARRANTIES OF MERCHANTABILITY, FITNESS FOR                
 * A PARTICULAR  PURPOSE, OR NON-INFRINGEMENT. KAPLAN, INC. SHALL                 
 * NOT BE LIABLE FOR ANY DAMAGES SUFFERED BY LICENSEE AS A RESULT OF                 
 * USING, MODIFYING OR DISTRIBUTING THIS SOFTWARE OR ITS DERIVATIVES.                
 *****************************************************************************/  
	 
 IF (@ProgramofStudyId <> 0)
   BEGIN
	SELECT ProgramID,
		ProgramName,
		[Description]
	FROM dbo.NurProgram
	WHERE DeletedDate IS NULL
	AND (@SearchText = ''
	OR ProgramName LIKE '%' + @SearchText + '%')
	AND ProgramOfStudyId  = @ProgramofStudyId
 END
 ELSE
 BEGIN
   SELECT ProgramID,
		ProgramName,
		[Description]
	FROM dbo.NurProgram
	WHERE DeletedDate IS NULL
	AND (@SearchText = ''
	OR ProgramName LIKE '%' + @SearchText + '%')
	AND ProgramOfStudyId IN (1,2)
 END
SET NOCOUNT OFF
END
GO
PRINT 'Finished creating PROCEDURE uspSearchPrograms'
GO 
