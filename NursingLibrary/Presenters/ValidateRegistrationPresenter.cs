...
{
    [Presenter]
    public class ValidateRegistrationPresenter : UnAuthenticatedPresenterBase<IValidateNursingView>
    {
        #region Fields
        ...
        private const string CodeNursingRN = "NursingIntegrated";
        private const string CodeNursingPN = "PNIntegrated";
        ...
        #region Constructor
        public ValidateRegistrationPresenter(IAdminService adminService)
        {
        ...
        public string GetCommandRegisterValues(ValidateNursingParams nursingParams)
        {
            ...
            for (int i = 0; i < listOfProducts.Length; i++)
            {
                ...
                if (!(productCode.Contains(CodeNursingRN) || productCode.Contains(CodeNursingPN)))
                {
                ...
                if (cpStatus == string.Empty)
                {
                    ...
                    if (!(string.IsNullOrWhiteSpace(nursingParams.FacilityId.Trim())))
                    {
                        institutionId = 
                            _adminService.GetInstitutionIdByFacilityIdOrClassCode(
                                Convert.ToInt32(nursingParams.FacilityId), classCode);
                    }
                    
                    studentInfo.Institution = new Institution() {InstitutionId = institutionId};
                    studentInfo.Group = new Group() { GroupId = 0 };
                    studentInfo.Cohort = new Cohort() { CohortId = 0 };

                    if (productCode == CodeNursingRN || productCode == CodeNursingPN)
                    {
                        nUserId = Convert.ToInt32(_adminService.GetUser(nursingParams.UserId, institutionId));
                        if (nUserId > 0)
                        {
                            createNew = false;
                        }
                    ...
                    if (createNew == true)
                    {
                        ...
                        _adminService.SaveUser(studentInfo);
        ...
        {
            if (nursingParams.ProductCode == CodeNursingRN || nursingParams.ProductCode == CodeNursingPN)
            {
                ...
                int results = Convert.ToInt32(_adminService.GetUser(nursingParams.UserId));
                if (results > 0)
                {
                    SessionManager.Set("UserID", results);
